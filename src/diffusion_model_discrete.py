...
from lightning_habana.pytorch.strategies.single import SingleDeviceStrategy
...
class DiscreteDenoisingDiffusion(pl.LightningModule):
    def __init__(
        ...
        if cfg.model.transition == "uniform":
            self.transition_model = DiscreteUniformTransition(
                x_classes=self.Xdim_output, e_classes=self.Edim_output, y_classes=self.ydim_output
            )
            ...
            self.limit_dist = utils.PlaceHolder(
                X=x_marginals, E=e_marginals, y=torch.ones(self.ydim_output) / self.ydim_output
            )
    ...
    def validation_step(self, data, i):
        ...
        with torch.autocast(device_type="hpu", dtype=torch.float16, enabled=False):
    ...
    def training_step(self, data, i):
        bench_dict['training_step'].gstep()
        with torch.autocast(device_type="hpu", dtype=torch.float16, enabled=False):
            bench_dict['training_step'].step('casting')
            ...
            bench_dict['training_step'].step('to dense')
            dense_data = dense_data.mask(node_mask)
            bench_dict['training_step'].step('mask')
            X, E = dense_data.X, dense_data.E
            noisy_data = self.apply_noise(X, E, data.y, node_mask)
            bench_dict['training_step'].step('apply noise')
            extra_data = self.compute_extra_data(noisy_data)
            bench_dict['training_step'].step('compute extra')
        #noisy_data = {k:v.bfloat16() for k,v in noisy_data.items()}
        #extra_data.X = extra_data.X.bfloat16()
        #extra_data.E = extra_data.E.bfloat16()
        #extra_data.y = extra_data.y.bfloat16()
        #pred = self.forward(noisy_data, extra_data, node_mask.bfloat16())
            pred = self.forward(noisy_data, extra_data, node_mask)
            bench_dict['training_step'].step('forward')
            ...
            bench_dict['training_step'].step('loss')
            self.train_metrics(
                masked_pred_X=pred.X,
                masked_pred_E=pred.E,
                true_X=X,
            ...
            print(loss)
            bench_dict['training_step'].step('end')
            bench_dict['training_step'].gstop()
        return {"loss": loss}
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        # Log val nll with defaultpytorch_lightning logger, so it can be monitored by checkpoint callback
        val_nll = metrics[0]
    ...
    def kl_prior(self, X, E, node_mask):
        ...
        return diffusion_utils.sum_except_batch(kl_distance_X) + diffusion_utils.sum_except_batch(
            kl_distance_E
        )
    ...
    def sample_batch(
        ...
        if self.visualization_tools is not None:
            ...
            current_path = os.getcwd()
            result_path = os.path.join(
                current_path, f"graphs/{self.name}/epoch{self.current_epoch}_b{batch_id}/"
            )
    ...
    def compute_extra_data(self, noisy_data):
        """ At every training step (after adding noise) and step in sampling, compute extra information and append to
            the network input. """
        bench_dict['compute_extra_data'].gstep()
        extra_features = self.extra_features(noisy_data)
        bench_dict['compute_extra_data'].step('extra_features')
        extra_molecular_features = self.domain_features(noisy_data)
        bench_dict['compute_extra_data'].step('domain_features')
        extra_X = torch.cat((extra_features.X, extra_molecular_features.X), dim=-1)
        extra_E = torch.cat((extra_features.E, extra_molecular_features.E), dim=-1)
        extra_y = torch.cat((extra_features.y, extra_molecular_features.y), dim=-1)
        bench_dict['compute_extra_data'].step('cat')

        t = noisy_data["t"]
        extra_y = torch.cat((extra_y, t), dim=1)
        bench_dict['compute_extra_data'].step('cat2')
        bench_dict['compute_extra_data'].gstop()
